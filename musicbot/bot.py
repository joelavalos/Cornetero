...
class MusicBot(discord.Client):
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, msg_by_nigga)
        self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, msg_nigga_disconnect)

        author_name = str(author)
        #self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, new_msg.split("#")[0])

        if author_name.split("#")[0] == "Glasstable":
            self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel,  "A sus ordenes mi amo y señor")

        if author_name.split("#")[0] == "Juanito de las nieves":
            self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Cancion ql penca ql xd")

        if author_name.split("#")[0] == "gizzybe":
            self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Hi you bastard, searching...   (come pulpos)")

        if author_name.split("#")[0] == "Carlitos Lechuga":
            self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Feo ql, cancion ql punga que pusiste wn o xd")
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                ...
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Nombre  : **%s**" % (player.current_entry.title))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "DJ      : **%s**" % (player.current_entry.meta['author'].name))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "LLeva   : **%s**" % (song_progress))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Quedan  : **%s**" % (song_total))
            else:
                #lines.append("Now Playing: **%s** %s\n" % (player.current_entry.title, prog_str))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Nombre  : **%s**" % (player.current_entry.title))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "DJ      : **%s**" % ("DJ Cornetero"))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "LLeva   : **%s**" % (song_progress))
                self.server_specific_data[channel.server]['last_np_msg'] = await self.safe_send_message(channel, "Quedan  : **%s**" % (song_total))
